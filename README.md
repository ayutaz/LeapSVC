...
> **学習済みの SVC 重みは、現時点では配布していません** — 学習素材のライセンスが未解決の
> ためです。**解決すれば公開を検討しますが、可否も時期も約束できません**
> （[ライセンス](#ライセンス)）。
...
| 低い声からの移調 | **+7 半音**（`SVC_TRANSPOSE_LOW_VOICE`） | 掃引で決定。**それまでの +12 は両方の軸で劣ります**（回復率 90.8% → 81.6%、CER の上限との差 +12.5 → +27.0 点）。**適用範囲は日本語の低音話者です。** 2026-09-18 に VocalSet の男声 11 本で測り直すと、**男声では +12 のほうが良い**（回復率は差が無く、明るさは +12 が target 本人と 13 Hz で一致。+7 は 332 Hz 遠い）。**必要な移調量は source の F0 に依るので、固定の定数では賄えません** |
...
**商用利用可能なモデルとして配るには何が要るか**を調べた結果は `doc/commercial-release-data.md` にあります（2026-09-19 調査）。要点は 2 つで、**塞いでいるのはデータの量ではなく権利**（同梱ボコーダー NHVSing の重みが非商用、base の素材 GTSinger が CC BY-NC-SA）であり、**商用可の歌唱素材だけでも現在の base と同規模（実測 29.2 時間 / 40 話者。由来に懸念のある SingVERSE を除いても 20.1 時間 / 23 話者）は揃う**ので、**同等品質の作り直しは素材の面では可能**です。ただし品質を上げる規模（100 時間級）には届きません。
...
- No.7（「小岩井ことり歌唱データベース」由来。楽曲は小岩井ことり氏に帰属／商用は No.7 製作委員会。**未知話者の評価用**。学習には使っていません） — https://voiceseven.com/
