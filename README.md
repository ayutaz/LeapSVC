...
- **速さ** — Reverse stepは1回のみであり、1コアのCPUで生成を行ってもRTFは0.03を切ります。なお、ステップ数を増やすことはできますが、1step以上行うとかえってGround truthから離れていきます。（**これは既存の歌声合成（SVS）経路の話**です。実験中の **SVC 経路は掃引の結果 16 step を既定にしています** — 明るさは 1 step と同等ですが、細部と話者性は 16 step が明確に上でした。`tools/svc_defaults.py` を参照）
...
**この表は既存の SVS 音響モデル単体の値です。** 実験中の **SVC 経路の end-to-end は別物**で、実測では **GPU で合計 RTF 0.464、うちボコーダーが 0.432（93%）**、acoustic は 0.006 でした（CPU では合計 0.654）。**acoustic を速くしても end-to-end はほとんど動きません。** また **`realtime_capable` が True でも「リアルタイム」とは書きません** — chunk 境界・audio I/O・連続運転を測っていないためです。
...
`feature/svc` ブランチでは、事前計算した content feature・F0・V/UV・音量から
既存の harmonic excitation と rectified flow を駆動する、オフライン歌声変換モデルを
開発しています。既存の音素＋durationによる歌声合成経路は変更しません。

**学習と評価は一通り完走しています。** content encoder は **ContentVec**（768 次元 layer 12、
学習には固定ランダムに選んだ 256 次元）、F0 は **RMVPE**、SSL から mel grid への整列は
**left（直前保持）**に決めています。WAV からコマンド 1 本で特徴量 shard を作れ（再実行で
bit 一致）、**23 話者・約 18 時間の multi-singer base 事前学習（60,000 step）**と、そこからの
**target singer（波音リツ）への fine-tune**まで通しました。**日本語の素材は 7 名・約 21.9 時間**
です（東北きりたん / No.7 を 2026-09-17 に追加）。

話者類似度は較正済みの話者照合モデル（**ECAPA-TDNN、12 秒以上のクリップ**）で測れるように
なりました。fine-tune で target らしさが上がることを確認しています（**VocalSet の未知 source**
で下限からの回復率 45.1% → 54.9%。**日本語の未知話者**では後述の 90.4%）。

**Seed-VC との客観比較を実施しました**（26 clip、同一条件）。**話者類似度は Seed-VC と
ほぼ同等**で（訂正後 0.5899 対 0.5912。旧 0.4981 は上限混入による測定誤り）、
**blind preference で負けているため**、事前登録した判定規則により **「Seed-VC より良い」とは
書けません**。一方 **F0 追従（相関 0.9994）・V/UV 一致（99.0%）・timing（73.7% 対 70.8%）は
LeapSVC が上**で、F0 を条件として直接与える設計の表れです。

**当時「話者性が弱い」と見えていた原因は過平滑でした**（予測 mel の細部が 25% 欠けていた）。
切り分けの結果、ボコーダーと mel 表現は無罪、source 話者の漏れも無し、話者条件も機能しており、
**GAN を有効にした fine-tune で未知 source の到達率が 69.4% → 75.3%**（Seed-VC 77.1%）へ
改善しました。**内容保持と F0 追従を落とさずに**です。**なお GAN は明瞭度を変えません**
（後述の切り分け）。
**話者類似度は主観でも判別できませんでした**（2026-09-15、target 本人にどちらが似ているかを別の質問として聴取。6/12 ペアで中断し、判定 4 票すべてが「後に聴いた側」に張り付いたため**判定に至らず**）。客観の同等（0.5899 対 0.5912）と整合します。

**blind listening test（N=1 の非公式 preference、26 ペア）でも Seed-VC が選ばれました**（25 判定中 21、引き分け 1）。**差は未知 source に集中**し（20 ペアで 2 対 18）、**target 本人の hold-out では拮抗**します（6 ペアで 2 対 3・引き分け 1）。

**未知話者の日本語でも通用します**（2026-09-17、東北きりたん / No.7 の 20 clip）。音量を合わせた条件で **明瞭度は上限との差 +7.3 点、話者類似度の回復率 90.4%、明るさは上限比 0.93x** で、**既知話者（+8.9 点 / 90.8%）と同等以上**でした。**ただし取得した 2 名は女声で target とほぼ同音域**なので、**男声や低音の未知話者は測れていません**。

**品質の律速を切り分けました。** `num_steps`・GAN の有無・fine-tune の step 数・base の学習量・**話者の既知性**のいずれも明瞭度を動かさず、**動くのは「その曲を学習で見たか」だけ**でした（学習曲は上限比 +1.0 点、未知曲は +7.3〜+11.1 点）。**したがって base を作り直す計画は実施しません。** なお **`eval/loss` は明瞭度の代理になりません**（継続学習で 0.02311 → 0.01459 と下がっても明瞭度は不変）。

**既定を 2 つ更新しました。** 推論の flow step は **16**（2026-09-01 の掃引）、低い声からの移調は **+7 半音**（2026-09-16 の掃引。それまでの +12 は話者類似度も明瞭度も劣ります）。**持ち込み音源には `--match-loudness` を付けてください** — 配信用に限らず**プロのスタジオ録音でも**要ります。

**リアルタイム student は未実施です**（2026-09-14 に直列経路から外し、起動条件つきにしました）。
