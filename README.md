...
**商用利用可能なモデルとして配るには何が要るか**を調べた結果は `doc/commercial-release-data.md` にあります（2026-09-19 調査）。要点は 2 つで、**塞いでいるのはデータの量ではなく権利**（同梱ボコーダー NHVSing の重みが非商用、base の素材 GTSinger が CC BY-NC-SA）であり、**商用可の歌唱素材だけでも現在の base と同規模（約 39 時間 / 24 話者以上）は揃う**ので、**同等品質の作り直しは素材の面では可能**です。ただし品質を上げる規模（100 時間級）には届きません。
