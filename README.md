...
（再実行で bit 一致）。実音声で overfit して WAV を出し、**23 話者・約 18 時間の multi-singer base 事前学習（60,000 step）**と、
そこからの **target singer への fine-tune（20,000 step）**まで通しました
（未知の source singer を入れても内容が崩壊しないことを実測）。
話者類似度は較正済みの話者照合モデル（ECAPA-TDNN、12 秒以上のクリップ）で測れるようになり、
fine-tune によって target らしさが上がることを確認しています（下限からの回復率 45.1% → 54.9%）。

**Seed-VC との客観比較を実施しました**（26 clip、同一条件）。**話者類似度では Seed-VC が
とほぼ同等で**（訂正後 0.5899 対 0.5912。旧 0.4981 は上限混入による測定誤り）、**blind preference で負けているため**、事前登録した判定規則により **「Seed-VC より良い」とは
書けません**。一方 **F0 追従（相関 0.9994）・V/UV 一致（99.0%）・timing（73.7% 対 70.8%）は
LeapSVC が上**で、F0 を条件として直接与える設計の表れです。

**話者性が弱い原因は過平滑でした**（予測 mel の細部が 25% 欠けていた）。切り分けの結果、
ボコーダーと mel 表現は無罪、source 話者の漏れも無し、話者条件も機能しており、
**GAN を有効にした fine-tune で未知 source の到達率が 69.4% → 75.3%**（Seed-VC 77.1%）へ
改善しました。**内容保持と F0 追従を落とさずに**です。
