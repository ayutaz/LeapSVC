...
QUESTIONS: dict[str, dict[str, Any]] = {
    ...
    # **名指しされた defect を直接聞く。** 2026-09-13 の聴取で 6 本中 5 本が
    # 「音量が揺れる」を挙げました。preference で聞き直すと、**差が「良さ」に
    # 化けて記録されます**（選択肢に無い defect が近いラベルに化ける、というのは
    # 実際に踏んだ失敗）。客観測定（`tools/loudness_stability.py`）では
    # **V3.1 と V3.2 に差が出なかった**ので、聴いて確かめるための質問です。
    "wobble": {
        "title": "blind wobble test（音量の揺れ）",
        "lead": "同じ 1 本の歌を、2 つの系がそれぞれ変換した結果です。"
                "どちらがどの系かは表示されません。",
        "note": ('<b>聞くのは「音量が揺れるか」だけです。</b> '
                 '伸ばした音の中で<b>大きさが細かく上下する</b>ほう、'
                 '<b>フレーム単位でぶつぶつ弱まる</b>ほうを選んでください。<br>'
                 '<b>全体の大きさの違いは揺れではありません</b> ―― '
                 'どちらが大きいか・明るいかでは選ばないでください。'
                 '<b>うまさ・きれいさでも選ばないでください</b>（別の質問です）。<br>'
                 '<b>引き分けは空欄ではなく tie。</b> 集計が区別します'
                 '（差が無かったこと自体が結果です。'
                 '<b>客観指標では差が出ていません</b>）。'
                 '<code>key.json</code> は集計まで開かないでください。'),
        "vote_a": "A のほうが音量が揺れる", "vote_b": "B のほうが音量が揺れる",
        # **前の 2 つと別の名前空間。** 同じにすると前回の票が復元されます。
        "storage": "leapsinger-blind-votes-wobble",
        "sheet": "out/m5/blind_wobble/sheet.csv",
