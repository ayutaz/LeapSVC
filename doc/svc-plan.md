...
条件ではありません）。**M6 は未着手**です。

**条件付きトラック (C)（GAN）は案 A まで実施済み**で、案 B（base から学習し直す）は未実施です。

**残っている直列経路は M6 だけですが、M5 の測定で M6 の前提が 1 つ崩れました**
（GPU の end-to-end RTF の 93% がボコーダー）。ここから何を選べるかは
...
| **M6** | streaming student | 実機で実時間動作させる | 実測 end-to-end latency と連続運転記録（**前提が変わりました**。[2b 節](#2b-ここからの計画)の④） |
...
**2026-09-14 時点。** M5 まで終わったので、残りの直列経路は M6 だけです。ただし M5 の測定で
**M6 の前提が 1 つ崩れました**（④）。ここでは、いま選べる作業を「**何に答えるか**」と
「**費用**」で並べます。**どれも必須ではありません** — 順序は目的次第です。
...
| **④** | **M6 の前に、ボコーダーの実行経路を測る** | 「M6 はそもそも成立するのか」 | GPU 1 時間 | 現状の ONNX(CPU) 実行を基準にする |
| ⑤ | 特徴抽出前の pitch augmentation | F0 → スペクトル傾斜の結合をモデル側で緩められるか | 前処理の作り直し + base 再学習 | **未実装**。`--transpose` で実用上は回避できている |

**推奨: ① → ②。** ①は費用ゼロで、**いまの決定（target 忠実）が測れているか**を確かめます。
②は「未知話者への転移」という**残った唯一の明確な弱点**に直接効く可能性が最も高い手です。
③と④は目的（品質を上げたいのか、実時間で動かしたいのか）が定まってからで構いません。
