...
**確認済み（2026-08-30 / 31、M3）:** GAN 無効のまま 23 話者・約 18 時間の base を **60,000 step** 学習し、hold-out の eval/loss は 0.02932 → 0.02311（30k）→ **0.01459（60k）**と単調に下がり続けました。`eval/varL`（GT に対する鮮鋭さの比、1.0 が同等）は 1.200 → 1.064 → **1.018** です。

**GAN を入れる前に、まだ回帰損失だけで伸びしろがあります。** 60,000 step でも eval/loss は 下降中で、**1 step と多 step の乖離も 10.8 → 6.0 点に縮みました**（[実行計画](svc-plan.md) M3）。GAN の導入は条件付きトラック C の判断に従います。
