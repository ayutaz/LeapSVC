...
### seed 0 と seed 1 の比較（2026-08-30 実測、M3 の base 学習の前に実施）

**確認済み:** 波音リツ 0.75 h（232 phrase、train 192 / hold-out 40）の同じ cache から 2 段目だけを
seed 0 / seed 1 で作り直し、**同じ config・同じ更新 budget（3,000 step）**で 2 本学習しました。
違うのは 256 次元の部分集合だけです。

| | eval/loss | eval/flow | eval/recon | mel MAE vs GT | F0 相関 | V/UV |
|---|---:|---:|---:|---:|---:|---:|
| seed 0（既定） | 0.02199 | 0.00794 | 0.02810 | 0.3755 | 0.9991 | 0.9956 |
| **seed 1** | **0.02029** | **0.00706** | **0.02645** | **0.3564** | 0.9992 | 0.9964 |
| 差 | **-7.7%** | -11.1% | -5.9% | -5.1% | ほぼ同じ | ほぼ同じ |

**seed 1 のほうが良い方向に一貫していました**（hold-out 40 phrase の損失と、1 phrase の
mel MAE の両方で）。F0 追従と V/UV は差がありません。

**ただし、これで seed 1 が良いと決められません。** 各 seed につき 1 run しか回しておらず、
**部分集合の差と run ごとのばらつきを分離できていません**。Interspeech 2025 の 2 つの部分集合も
SSIM 0.813 / 0.822 と小さい差でした。

**決定:** M3 の base 学習は**既定の seed 0 で進めます**（比較の基盤を途中で変えないため）。
seed 1 は候補として残し、**M4 の前に seed を振り直した反復で確かめます**。それまでこの数値を
根拠に「seed 1 のほうが良い」とは書きません。

**追記（2026-09-01）: この反復は実施されないまま M4 まで進みました。** seed 0 を既定として
据え置いています。**したがって「seed 1 のほうが良い」は依然として書けません。** 反復を回すなら
2 段目（`--from-cache`）だけで済み、ContentVec と RMVPE の再実行は要りません
（[実装状況](svc-implementation-status.md) の「未検証」）。
