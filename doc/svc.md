...
**未完了なのは、明瞭度を読める素材とリアルタイム student です。** 明瞭度（CER）は
**上限がボコーダーの乱数で run ごとに変わる**ため系間比較が成立していませんでしたが、
[上限を使い回す仕組み](svc-plan.md#2b-ここからの計画)（2026-09-15 に実装）で**測り直しました** ――
**上限の食い違いは 0 / 26** になり、**読める素材では GAN と 16 step が明瞭度を変えていない**
ことが分かりました（+9.9 → +9.9 点、改善 3 / 悪化 4 / 同じ 7）。**残る問題は素材です:**
読めるのは 10 本だけで、**うち 6 本は target 本人の自己再構成**です。**日本語の未知 source**
を足すまで、明瞭度の劣化の原因（content 側か音響側か）には踏み込めません。
