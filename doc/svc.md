...
**利用者が持つ 1,000 時間級の日本語素材（YouTube 由来）は、投入を保留しました。**
事前登録した 2 つの判定（[実行計画](svc-plan.md) 12 節）が**両方とも不合格**で、
**学習は 1 本も回していません** ―― 低音域は入っておらず（C3 未満の中央値 1.224%、n=75）、
分離ボーカルは**上限が既存素材と別物**になります（mel L1 0.5279 対 0.2929〜0.3371、
p = 0.000002）。**予測は 2 つとも外れ**、そのまま記録しています。
