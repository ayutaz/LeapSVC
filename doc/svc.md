...
**商用可の素材だけで作り直す試み（S1）も不合格でした（2026-09-21）。** 許諾を待たずに使える
16 話者 / 17.0 h（VocalSet・NIT-SONG070・波音リツ）で base を学習しましたが、
**明瞭度 +25.5 → +50.5 点、話者類似度 77.9% → 49.2%、明るさも悪化**（14 節）。
**律速は量ではなく「歌詞のある楽曲が無いこと」**という読みです ―― 商用可素材の中核 VocalSet は
**音階・技法練習で歌詞を含みません**。**したがって商用リリースには、歌詞のある楽曲の
商用可コーパス（自前収録か有償ライセンス）が要ります。**

**その仮説は統制実験で支持されました（S1b、2026-09-26）。** S1 と同じ 16 話者の形のまま、
非リツ 14 名の中身だけを**音階練習から歌詞つきの実楽曲**へ差し替えると、明瞭度は
**+50.5 → +34.2 点**（S1 から現 base 側へ **65% 戻る**）。ただし**話者類似度と明るさは戻らず**、
残る差は**1 話者あたりの量か録音の質**と見ています（15 節）。**買う（録る）べき素材は
「歌詞のある楽曲・スタジオ品質・1 歌手あたり十分な量」**で、現 base の形なら
**約 20 歌手 × 0.75 h ≈ 15 h**（収録費の見積もり ¥22 万〜100 万、歌唱料・権利は別）です。
