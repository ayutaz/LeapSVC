...
| 見込み | 根拠 |
|---|---|
| 低音域の男声を埋められる（**仮説。実測 n=1 では支持されず**） | C3 より下の滞在時間は**全素材で最大 3.6%**（[台帳](svc-dataset-ledger.md) 4b 節）。ところが**測った 1 本は 0.5%** でした（下の実測節）。**分布は未測定** |
...
2. **cover による曲の重複。** `cover` が 3/13 あり、**同じ曲が別の動画として複数入ります**。
   ...
   規約に合わせること。
3. **低音域は 1 本では埋まっていません。** F0 p50 284 Hz は既存素材の中間
   （夏目悠李 206 / リツ 360 / くるみ 395）ですが、**C3 より下の滞在は 0.5%** で、
   **夏目悠李の 3.6% より少ない**です。**n=1 なので分布は未測定**ですが、
   **「YouTube なら低音男声が入る」という仮説は、まだ支持されていません。**
4. **分離アーティファクト。** 分離 stem は**伴奏 leakage・位相 artifact・残響が target timbre
   として学習される危険**があり、既存方針は「元の isolated vocal と分離 stem は dataset tag を
   分け、同じ test set へ混在させない」です（[データと計算資源](svc-data-compute.md) 1 節）。
   **これは帯域では測れません。** 聴取か、分離前後の比較が要ります。
...
**問い合わせ文面のたたき台は [台帳](svc-dataset-ledger.md) 10 節にあります**（御丹宮くるみ /
夏目悠李 / 波音リツ 向け。No.7 向けは未作成）。
