...
| [`test_svc_preprocess.py`](../test_svc_preprocess.py) / [`test_svc_dataset.py`](../test_svc_dataset.py) | 契約テスト（合計 410 件のうち 178 件） |
...
| `out/m5/steps_sweep.json` | `num_steps` の掃引（1〜32、未知 source 20 clip）。**既定を 16 にした根拠** |
| [`configs/svc_target_ft_gan.yaml`](../configs/svc_target_ft_gan.yaml) | **GAN fine-tune の recipe**。M4 との差分は gan セクションのみ |
...
| `out/m5/blind/` | blind preference の材料（26 ペア、**音量を揃えた**もの）と聴取ページ `listen.html`、参照音声 `context/`（target 本人の録音 + 各ペアの変換元 26 本）。**投票済み（26/26）**。結果は `result.json`（25 判定中 21 が Seed-VC） |
