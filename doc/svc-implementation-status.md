...
- 自動テスト **563 件**が成功（`test_svc_model` 64 / `test_svc_preprocess` 143 / `test_svc_dataset` 100 / `test_svc_metrics` 256）。重いモデルもネットワークも使いません。実モデルの統合テストは 4 件で、`LEAPSINGER_INTEGRATION=1` のときだけ走ります。
...
- **男声・低音の未知話者**（**部分的に解消。2026-09-18**）。日本語では取得した 2 名が女声・
  target と同音域でしたが、**VocalSet の男声 11 本（未知話者）で移調を測り直しました** ――
  **+7 は男声へ広げられず、+12 のほうが良い**（[実行計画](svc-plan.md) 11 節）。
  **日本語の男声と、C3 より下の滞在時間の欠落は未解消**です（低音を担うのは夏目悠李だけ）。
...
| 道具 | 役割 |
|---|---|
| [`tools/check_links.py`](../tools/check_links.py) | md のリンク切れとアンカー切れ。**CI と単体テストの両方で回る**（それまで scratchpad の使い捨てだったので、回し忘れると通った） |
