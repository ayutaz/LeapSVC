...
| [`tools/loudness_stability.py`](../tools/loudness_stability.py) | 追加済み | 音量の揺れ（source 基準なので**系をまたげる**）。速い成分と遅い成分を分ける。**聴取で名前が付いた defect を客観化したが、再現しなかった** |
| [`tools/blind_test.py`](../tools/blind_test.py) | 追加済み | blind preference の材料作成・**聴取ページ生成**・**defect 名付けページ**・集計。system 名を隠し、順序を randomize。**上限を参照に渡すと落ちる**（ボコーダーの癖で系が割れるため） |
...
- ボコーダーは **NHVSing V3.1**（`checkpoints/nhv_v3_1.onnx`。2026-09-13 に上流から取り込み）。**M5 の測定値は V3 で取ったもの**なので、V3.1 で測り直した値と混ぜないこと。**2026-09-18 に上流の V3.2 を取り込みましたが（`main` への merge）、SVC の既定は V3.1 据え置きです** — 上限がボコーダーごとに変わるため、移行は測り直しとセットで行います。V3.2 は高音域でフレーム単位に波形が急激に弱まる現象を直したもので、**聴取で報告された「音量が揺れる」に対応する可能性があります（仮説）**。
- 自動テスト **498 件**が成功（`test_svc_model` 58 / `test_svc_preprocess` 115 / `test_svc_dataset` 82 / `test_svc_metrics` 243）。重いモデルもネットワークも使いません。実モデルの統合テストは 4 件で、`LEAPSINGER_INTEGRATION=1` のときだけ走ります。
...
| 指標 | LeapSVC | Seed-VC |
|---|---:|---:|
| 話者類似度（回復率） | 0.4712（50.3%）**←測定誤り。訂正後 0.4749（51.2%）** | **0.5912（80.2%）** |
...
**訂正（2026-09-15）: CER の +0.168 は読めない値でした。** 言語別に割ると **日本語 +5 点
前後 / 英語 +10〜19 点**で、pooled の値は**上限自身が 23% 揃わないラテン語**と n=1 の
イタリア語に引き上げられていました。さらに**上限（`*_vocoder_only.wav`）は run ごとに
変わります** ―― NHVSing の ONNX に seed 属性の無い `RandomNormalLike` があるためで、
**同じ V3 の 2 つの run で 26 clip 中 6 本の上限 CER が 5 点超ずれました**（最大 88 点）。
`tools/svc_convert.py --ceiling-from` で上限を使い回せるようにしてあります。
...
（0.2643、下限 0.2696）、話者条件も機能（target 指定で 0.24 → 0.52）。**予測 mel の
ラプラシアン分散比が 0.755**（細部が 25% 欠けている）で、**base も fine-tune も
`gan.enabled: false`** でした。**step を増やすと細部と話者性が同時に戻る**ことで
因果を確認しています（16 step で細部比 0.957 / 回復率 90.6%）。詳細は
[実行計画](svc-plan.md) の調査節、一次データは `out/m5/probe{1..6}_*.json`。
...
### 未知話者の日本語での実証（2026-09-17）

**確認済み:** 東北きりたん / No.7 を取得し（**base に入っていない 2 名**）、20 clip・
各 20 秒・移調 0・`--match-loudness` ありで波音リツへ変換しました。一次データは
`out/m5/ja_unseen_gan_ml/` と `out/m5/sweep_metrics/{cer,sim}_ja_unseen_ml.json`。

| 層（すべて同じ系・日本語） | 明瞭度（上限との差の中央値） | 回復率 | 明るさ（上限比） |
|---|---:|---:|---:|
| **学習した曲そのもの** | **+1.0 点** | ― | ― |
| 未知曲・自己再構成 | +10.7 点 | 91.4% | 1.02x |
| 未知曲・既知話者（夏目 +7） | +8.9 点 | 90.8% | 0.95x |
| **未知話者**（きりたん / No.7） | **+7.3 点** | **90.4%** | **0.93x** |

**未知話者は既知話者と同等以上**です。**話者の既知性はほとんど効いておらず、明瞭度で
動くのは「その曲を学習で見たか」だけ**でした。

**限界:** 取得した 2 名は**女声で target とほぼ同音域**（336 / 324 Hz 対 360 Hz）、
**C3 より下は 0.0%** です。**男声や低音の未知話者は測れていません。**
...
- ~~CER の日本語歌唱での信頼性~~ — **改善（2026-09-16〜17）。** 日本語の test set を
  作り（`natsume` / `oniku` の未使用曲 50 clip、きりたん / No.7 の 20 clip）、**上限 CER は
  中央値 0.0〜2.6%**、**除外ゼロ**で安定しています。**ただし CER は離散で不安定**です ――
  1 clip で +88.7 点、+364.2 点といった暴走が出ます（`cer_breakdown.py` が除外します）。
- **CER の「上限との差」は 2 通りある**（**中央値の差**と**差の中央値**）。`asr_cer.py` が
  両方を出します（`cer_excess_diff_of_medians` / `cer_excess_median_of_diffs`）。
  **比較には差の中央値を使うこと。** 2026-09-17 に**混ぜて引用していた**のを訂正しました。
- **男声・低音の未知話者**。取得した 2 名は女声・target と同音域でした。**低音を担うのは
  夏目悠李だけ**で、**C3 より下の滞在時間がどの素材にもほぼ無い**という欠落は解消して
  いません。
...
### 2026-09-14〜17 に足した道具（確認済み）

| 道具 | 役割 |
|---|---|
| [`tools/cer_breakdown.py`](../tools/cer_breakdown.py) | CER を**言語ごとに割る**。上限中央が 10% を超える群と 3 本未満の群、**変換 CER が 100% を超えた clip**（ASR の暴走）には差を出さない |
| [`tools/ja_material_audit.py`](../tools/ja_material_audit.py) | 日本語素材の**曲単位の棚卸し**。**相対パスで照合**（ファイル名では全部が「学習済み」に見える）。`--layout` は必須 |
| [`tools/ja_testset.py`](../tools/ja_testset.py) | 日本語 test set の構築。**学習に入った曲を絶対に混ぜない**。`--seen-speaker` / `--unseen-speaker` が必須（**層を決め打ちしない**） |
| `svc_convert.py --ceiling-from` | **上限を作り直さず使い回す。** NHVSing が seed の無い乱数を持つため（**CPU + 決定的モードでも一致しません**）。`convert.json` が `ceiling_comparable` を申告 |
| `svc_convert.py --deterministic` | acoustic 側だけを決定的にする（**出力 WAV は bit 一致しません**） |
| `blind_test.py page --question similarity` | 「どちらが target に似ているか」を**別の質問**として聴く。票の保存先も分かれる |
| `blind_test.py prepare --anchor-target/--anchor-foil` | **anchor（catch trial）**。正解は `anchors.json` にだけ置き、`tally` が**合格線 75%** で読み方の門にする |
| `tally` の `sides` / `p_side` | **位置の偏り**を出す。**p_side が系の p より小さいときは系の勝敗を読む前に疑う** |
| `tools/svc_defaults.py` の `SVC_TRANSPOSE_LOW_VOICE` | 低い声からの移調量 **+7**（掃引で決定） |
...
10. ~~target 類似の blind~~ — **実施済み（2026-09-15）。判定に至らず**（判定 4 票すべてが「後に聴いた側」）。
11. ~~CER の分解~~ — **1 を実施済み（2026-09-15）。** 公表値 +0.168 は言語の混合で、訂正後は
    **日本語 +5 点前後 / 英語 +10〜19 点**。ただし**上限が run をまたいで再現しない**ため系間比較は未成立。
12. ~~上限を 1 度だけ作って共有する~~ — **完了（2026-09-15、1a）。** `svc_convert.py --ceiling-from`
    で作り直さずに使い回す。`convert.json` が `vocoder` / `vocoder_stochastic` /
    `ceiling_source` / `ceiling_comparable` を記録する。**原因はボコーダーの乱数**
    （seed 属性の無い `RandomNormalLike`）で、CUDA でも決定的モードでもなかった。
...
18. ~~案 B: base から学習し直す~~ — **実施しないと決めました（2026-09-17）。**
    **未知話者でも既知話者と同等**（明瞭度 +7.3 対 +8.9 点、回復率 90.4% 対 90.8%、n=20）で、
    **話者の既知性がほとんど効いていない**ため。`num_steps`・GAN・fine-tune の step・
    base の学習量のいずれも明瞭度を動かさず、**動くのは「その曲を学習で見たか」だけ**でした。
