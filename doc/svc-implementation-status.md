...
- 自動テスト **463 件**が成功（`test_svc_model` 57 / `test_svc_preprocess` 115 / `test_svc_dataset` 63 / `test_svc_metrics` 228）。重いモデルもネットワークも使いません。実モデルの統合テストは 4 件で、`LEAPSINGER_INTEGRATION=1` のときだけ走ります。
...
12. **上限を 1 度だけ作って共有する**（+ `convert.json` にボコーダーを記録）。**次にやること。**
    上限は GT mel とボコーダーだけで決まるので、変換の run ごとに作り直す必要がない。
13. **主観テストに anchor（catch trial）を入れる**（「差が無い」と「判別できていない」を分ける。GPU 不要）。
14. **男声 source の transpose 掃引**（+5 / +7 / +9 / +12。**再学習なし**。明瞭度が最も崩れる条件）。
15. **案 B: base から GAN 付きで学習し直す**（条件付きトラック C の残り。8〜12 時間。**要ユーザー判断**）。
16. ~~ボコーダーの実行経路の実測~~ / ~~streaming student~~ — **後回し（2026-09-14 決定）。**
    **M6 は直列経路から外し、品質が十分になってから**着手します（must ではありません）。
    着手するときの最初の作業は蒸留ではなく**ボコーダーの実行経路の測定**です
    （[起動条件](svc-plan.md#m6-streaming-student)）。

12〜15 の位置づけと推奨順（**1a → 2a**）は [実行計画](svc-plan.md#2b-ここからの計画) の 2b 節にあります。
