...
- 自動テスト **514 件**が成功（`test_svc_model` 64 / `test_svc_preprocess` 115 / `test_svc_dataset` 82 / `test_svc_metrics` 253）。重いモデルもネットワークも使いません。実モデルの統合テストは 4 件で、`LEAPSINGER_INTEGRATION=1` のときだけ走ります。
