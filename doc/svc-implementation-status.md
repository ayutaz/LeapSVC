...
- 自動テスト **494 件**が成功（`test_svc_model` 58 / `test_svc_preprocess` 115 / `test_svc_dataset` 82 / `test_svc_metrics` 239）。重いモデルもネットワークも使いません。実モデルの統合テストは 4 件で、`LEAPSINGER_INTEGRATION=1` のときだけ走ります。
