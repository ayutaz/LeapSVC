...
- 自動テスト **532 件**が成功（`test_svc_model` 64 / `test_svc_preprocess` 130 / `test_svc_dataset` 82 / `test_svc_metrics` 256）。重いモデルもネットワークも使いません。実モデルの統合テストは 4 件で、`LEAPSINGER_INTEGRATION=1` のときだけ走ります。
