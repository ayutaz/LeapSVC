...
- 自動テスト **590 件**が成功（`test_svc_model` 64 / `test_svc_preprocess` 146 / `test_svc_dataset` 112 / `test_svc_metrics` 268）。重いモデルもネットワークも使いません。実モデルの統合テストは 4 件で、`LEAPSINGER_INTEGRATION=1` のときだけ走ります。
