...
単体テストは **590 件**（`test_svc_model` 64 / `test_svc_preprocess` 146 / `test_svc_dataset` 112 / `test_svc_metrics` 268）で、重いモデルもネットワークも使いません。`unittest discover` は hook で止めています（収集条件が暗黙で、走った件数が分かりにくいため）。上の 4 本を明示的に並べるか、`run_smoke.py` の `unittest` ステージを使ってください。後者は top-level の `test_*.py` を自動収集し、件数を表示します。`uv` を介さず素の Python で走らせると `librosa` 等が無く収集時に失敗します。
...
**Python の通常の三重引用符でも同じ壊れ方をします。** `"""…"""` の中では**行末の `\` が
文字列リテラルの行継続**として解釈され、`\` と改行の両方が消えます（README のコマンド例
12 本が 1 行に連結されました）。**`r"""…"""`（raw string）で組み立て、書いた後に行継続の
本数を数えて確認すること。**
...
| `tests` | **macOS** | `uv sync --locked` + 単体テスト 590 件 + ruff + リンク検査 |
...
- **CER の「上限との差」は 2 通りあります（2026-09-17 に取り違えました）。**
`asr_cer.py` の `cer_excess_over_ceiling` は **中央値の差**（median(変換) − median(上限)）、
`cer_breakdown.py` は **差の中央値**です。**名前が同じで中身が違い**、歪んだ分布では一致
しません（実測で +12.7 対 +10.7、+25.5 対 +18.8）。**比較には差の中央値を使うこと。**
`asr_cer.py` は両方を別の名前で出します（`cer_excess_diff_of_medians` /
`cer_excess_median_of_diffs`）。**旧い名前は旧い意味のまま**残してあります。
- **プロのスタジオ録音にも `--match-loudness` が要ります（2026-09-17 実測）。**
東北きりたん / No.7（Neumann u87ai、96 kHz）は学習分布から **中央値 +0.60 σ** ずれており、
**付けないと明るさが上限比 0.71x**、付けると **0.93x** になりました。**「持ち込み音源」は
配信用に限りません** ―― プロ録音でも学習素材（波音リツ DB は peak 0.107）から外れます。
**明瞭度の改善（+11.1 → +7.3 点）は clip 単位では p = 1.0 で確立しません**が、
**明るさは連続量なので確実**です。
- **未知話者の日本語でも既知話者と同等です（2026-09-17 実測、n=20）。** 音量を合わせると
**明瞭度 +7.3 点 / 回復率 90.4% / 明るさ 0.93x** で、既知話者（+8.9 点 / 90.8%）と同等
以上でした。**ただし女声・target と同音域**の 2 名なので、**男声や低音の未知話者は
測れていません。**
- **明瞭度で動くのは「その曲を学習で見たか」だけです（2026-09-17 実測）。**
学習した曲そのものは **+1.0 点**、学習していない曲は**話者が既知でも未知でも +8.9 〜 +11.1 点**
です（未知話者 n=20 で +11.1 点、回復率 88.5%）。**話者を変えても、移調を変えても、GAN を
入れても、step を増やしても動きません。** したがって **base に話者を足す価値は低い**と
見るべきです。
...
（中央値の差 1.7 点、clip 単位 3 対 3、p = 1.0）。
- **明瞭度は「学習の量」では動かず、「何に適応したか」で動きます（2026-09-17 実測）。**
`num_steps`・GAN の有無・fine-tune の step 数・base の学習量を変えても**動きません**が、
**target への適応**（base +25.5 点 → fine-tune +12.7 点）と**学習曲か未知曲か**
（+1.0 点 対 +12.4 点）で**大きく動きます**。**したがって律速はデータの量と多様性です**
（base は 23 話者 18 時間、日本語は 5 名 19.82 時間）。**この規模では +12.7 点が水準という
可能性も残ります。**
...
この測定で否定されました。**
- **`cd` でリポジトリ直下から離れたままにしないこと。** hook は
`tools/hooks/guard_commands.py` を**セッションの作業ディレクトリ基準**で探すので、
`cd out/m5/demo` した後は見つからず、**Bash も PowerShell も一切使えなくなります**
（hook 自身が落ちるので、`cd` で戻すこともできません）。実際に踏みました。
**復帰方法:** Write ツールで `<いまの cwd>/tools/hooks/guard_commands.py` に**実体へ委譲する
shim** を置くと hook が通るので、`cd` でリポジトリ直下へ戻してから shim を消します
（guard の判定は変わりません）。**2026-09-20 に 2 度目を踏んで、この行の旧い助言
「複合コマンドの中だけで使えば安全」が誤りだと分かりました** ―― `cd .m0data/p0/nit && tar ...`
のように `&&` で繋いでも、**セッションの cwd は動きます**。**正しい回避は 2 つだけ**:
**絶対パスで指定して `cd` を使わない**か、**サブシェルに閉じる**（`(cd X && cmd)`）。
- **ログを日本語で grep して待たないこと。** `until grep -q "展開が終わりました" log; do sleep; done`
は**永久に回り続けました**。`uv run python > log` で落ちたログは **cp932**（Windows の
ロケール符号化）で、シェルから渡す **UTF-8 のパターンと一致しない**ためです。**終了は
ファイルの存在で待つ**（`until [ -f out.json ]`）か、**ASCII の印を出させる**こと。
`errors="replace"` で読むと化けるログは、**grep でも一致しません**。
...
- **上流の V3.2 を取り込みましたが、SVC の既定は V3.1 据え置きです（2026-09-18 決定）。**
上流の `66b464e` は **V3.1 を削除して V3.2 に差し替える**もので、main へ merge した時点で
`checkpoints/nhv_v3_1*.onnx` が消えました（**SVC の 16 ファイルが既定でこの名前を参照**）。
**V3.1 を同梱に戻し**（`.gitignore` の `!` 行も両方）、SVS 側だけ V3.2 を既定にしています。
理由は**上限がボコーダーごとに変わる**ことで、V3.2 へ移すと README と doc の「上限との差」が
**すべて測り直し**になります。**移行は測り直しとセットで行うこと。**
...
`frame_jitter_db` は **6/26・p = 0.0094 で有意に悪化**（ただし +0.035 dB で無視できる大きさ）。
**申告された 5 本でも改善せず**、**「高音ほど改善する」という予測も外れました**
（Spearman rho −0.221 〜 +0.045、p > 0.2）。**読めるのは「この測定では差が無い」まで**です。
一次データは `out/m5/_v32/`、判定規則は [`doc/svc-plan.md`](doc/svc-plan.md) 10 節。
**否定的な結果をそのまま残し、指標を足しませんでした** ―― 実データを見てから選ぶと
結果に合う指標を選ぶことになります。次は同じ clip を V3.1 と V3.2 で聴き直します。
- **「中央値の差」と「差の中央値」が符号ごと食い違う実例が出ました（2026-09-18）。** 上限の
`fast_db_std` は**中央値の差 −0.032（改善）**に対し**差の中央値 +0.023（悪化）**です。
**比較には差の中央値を使うこと**という規則が、実際に結論を分けました。
...
- **`min_bandwidth_hz=16000` は sample rate 由来の値で、mel 契約由来ではありません。** `mel` は 40–16,000 Hz / 128 bin なので、**最上位 bin の中心は 15,540 Hz** です（`librosa.mel_frequencies(n_mels=130, fmin=40, fmax=16000)`）。MP3 由来の素材は lowpass が約 15.8 kHz に来るので、**16,000 Hz を閾値にすると 92% が `band_limited` で弾かれるのに、mel では 1 bin も失われません**（実測 13 本で p50 15,781 Hz、中心が実効帯域より上の bin は 0/128）。**閾値は「素材に合わせて緩める」のではなく、上位 bin の中心という根拠で決めること。** 経緯は [`doc/commercial-release-data.md`](doc/commercial-release-data.md) 4.5 節。
- **gated な HF データセットは、展開せずに中身を測れます。** `hf_hub_url` + HTTP `Range` で tar の先頭だけ取り、512 バイトのヘッダを辿れば member 名・サイズが読めます。**200 GB のうち 40 MB で sample rate・bit 深度・実効帯域・peak・F0 分布まで分かりました**（`tts-dataset/japanese-singing-voice-vocal-only`）。**素材の採否を決める前に全部落とさないこと。**
- **予算で打ち切るループは「もう 1 本も入らない」で止めること。** `total >= budget` だけを終了条件にすると**端数が永遠に埋まりません**。実測（2026-09-20）で、30 時間の予算に対し **29.98 時間**まで埋めた後、残り 72 秒に収まる曲を探して **40 個の tar を約 1 時間ぶん streaming し続けました**（選択数は 473 のまま増えず）。**通信量がそのまま料金になります。** `tools/p1_corpus.py` の `budget_exhausted()` が「残り < 典型的な 1 本」で打ち切ります。
