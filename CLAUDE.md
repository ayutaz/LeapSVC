...
同一**です。**聴取で 6 本中 5 本が報告した「音量が揺れる」に対応する可能性があります**
（`tools/loudness_stability.py` では LeapSVC のほうが source に忠実で、原因が掴めていません）。
**これは仮説です** — 確かめるには V3.2 で上限から測り直し、同じ clip で聴き直す必要があります。
