...
```bash
uv run python -m unittest test_svc_model test_svc_preprocess test_svc_dataset test_svc_metrics -v   # 単体 590 件
...
## 3e. 自分で作った文字列だけで通したテストは、実データで落ちる（2026-09-20）

YouTube の題名から曲名を作る heuristic に 6 件のテストを書き、**全部通してから**実データ
13 本を通したところ、**2 件が誤り**でした。

| 何を想定したか | 実データ | 結果 |
|---|---|---|
| カバー表記は `歌ってみた` / `歌わせて` | **`うたった`**（ひらがな） | 装飾が剥がれず別の曲名になる |
| `Artist - Song` は区切って前半を採れば曲名 | `Misumi - アンダードッグ feat.flower` | **アーティスト名が曲名になり**、その歌手の全曲が 1 曲へ潰れる |

**2 つ目は既知の壊れ方と同型です**（GTSinger で 1,723 ファイルが 1 つの名前に潰れた）。
**自分の想像した入力だけでは、同じ壊れ方を 2 度踏みます。**

**規則:**

1. **実データのサンプルを 1 つ以上テストに入れる。** 文字列は短いので、実物をそのまま
   テストへ貼れます（`test_the_real_symag_title_folds_to_the_song`）。
2. **heuristic は「畳み過ぎない」方向に倒す。** 畳め損ねは「別の曲扱い」で済みますが、
   畳み過ぎは**全曲が 1 つに潰れます**。
3. **限界をコードとドキュメントに書く。** この heuristic は **cover の自動グルーピングを
   解決していません**。「未解決」と書いておけば、次の人が信用し過ぎません。

## 3f. 注入は後からでも足せる（2026-09-20）

`stage_extract()` は ContentVec と RMVPE を**関数の内側で**作っていました。話者が 3 桁に
なると、話者ごとに別プロセスで回すのは現実的ではありません（読み込みが話者の数だけ載る）。

**直し方は `*, encoder=None, f0x=None` を足すだけ**でした。

- **渡さなければ従来どおり自分で作る** —— 既存の呼び出しは 1 つも変わりません。
- 渡せるようになったので、**多話者版（`run_speakers`）は 1 度だけ作って使い回せます。**
- **テストは偽物を渡す**ので、ネットワークも GPU も使いません（8 件を先に書いて失敗を確認）。

**「テストが書きにくい」と「本番が遅い」は同じ原因から来ます。** 2 節の依存注入は、
テストのためだけの作法ではありません。
