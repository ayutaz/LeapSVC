...
## 6d. base の素材を差し替えるとき（2026-09-20、P1 で踏んだ 3 件）

**学習を始める前に 3 つの取り違えを見つけました。** どれも**例外にならず、測れてしまう**
形で壊れます。素材を差し替える実験では必ず確かめること。

### (1) 新しい base に「変換先の話者」が居るか

評価は「リツへ変換した hold-out 6 clip」で行うのに、**新しい corpus は YouTube の cover
歌手だけ**でした。**リツが居ないので `--spk-id` を指定できず、比較そのものが成立しません。**

**確かめ方:** 評価で使う `--spk-id` が、新しい `spk_map` に**存在するか**を学習前に見る。
**素材を差し替えると speaker id は総入れ替えになります。**

### (2) 評価に使う曲が、新しい corpus に混ざっていないか

比較相手（現 base）の hold-out 3 曲を、新しい corpus にそのまま入れると
**「学習した曲」を測る**ことになります。実測で**学習曲は上限との差 +1.0 点、未知曲は
+12.4 点**と桁が違うので、**混ざると新しい base が不当に良く出ます。**

**確かめ方:** hold-out 曲名は
`random.Random(42).sample(sorted(曲名), eval_songs)` で再現できます（曲名は **casefold して
から** `[^\w-]+` を `_` に置換した形）。**素材を配置するときに名前一致で除外する**。
別名版（`anywhere-3_normal` に対する `anywhere`）も落とすこと。

### (3) 素材をまとめるときの名前衝突

3 音源を 1 つの話者ディレクトリへ `cp -rn` でまとめたら、**同じファイル名が衝突して
150 本が 100 本に減りました**。`-n`（no-clobber）は**黙って飛ばします**。

**確かめ方:** **コピーの前後で本数を数える。** 声・技法・セッションをまたいで
まとめるときは、**接頭辞を付けてから**コピーする。

## 6e. 事前登録したゲートを利用者の判断で解除するとき（2026-09-20）

P0 の 2 つの判定は**両方とも不合格**でしたが、利用者の判断で **P1 を起動**しました。

- **判定は書き換えません。** 解除したのは**中止条件**であって、測定結果ではありません。
- **解除したことを、日付と理由つきで残します。** 事前登録は「結果を見てから規則を動かさない」
  ための規律なので、**利用者の意思決定で上書きされること自体は想定内**です。
- **期待値も書き直しません。** 「低音域は入っていない」と測れているなら、
  **その期待は落としたまま**進めます。
